 ...
 */
//#pragma omp task
void raytask(RayEngine* re, int start_sl, int sl_num) {
    int xres = re->getxres();
    int yres = re->getyres();

    for(int i = start_sl; i < start_sl + sl_num; i++) {
        re->render_scanline(xres, yres, i, re->getrpp());
    }
}
 ...
 */
void RayEngine::run() {
    int rem = yres % SL_BLOCK;
    int even = yres - rem;
    #pragma omp parallel 
    {
      #pragma omp single nowait
      {
	for(int i = 0; i < even; i += SL_BLOCK) {
	  #pragma omp task
	  {
	    raytask(this, i, SL_BLOCK);
	  }//end task
	}
	if(rem != 0) {
	  #pragma omp task
	  {
	    raytask(this, even, rem);
	  }//end task
	}
      }//end single
    }//end parallel
