...
class RotateEngine {
    public:
		...
		void run();
		void finish();
		bool init(Image* input, unsigned int angle, string destname, unsigned int numthreads, bool pinning);
        void printRotationState();
        void computeRow(int row, int rowwidth, float xot, float yot, float xos, float yos, unsigned rev_angle);
        /* This is for threading. I know it should be private and manipulated using getters and setters. */
        unsigned int sl;
        bool pinning;
    private:
        /* Variables */
...
/*
*   Parameter structure for rotation threads.
*/
typedef struct {
    RotateEngine *re;
    int rownum, rowcnt, rowwidth, imagerows;
