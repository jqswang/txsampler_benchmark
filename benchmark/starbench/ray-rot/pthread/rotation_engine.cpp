...
*/
bool RotateEngine::init(Image* input, unsigned int angle, string destname, unsigned int numthreads, bool pinning) {
    this->angle = angle;
...
void* rethread(void* args) {
    rotate_threadarg_t* arg = (rotate_threadarg_t*)args;
...
//             fprintf(stderr, "Writing pel (%d, %d, %d)\n", final.r, final.g, final.b);
            /* Write output */
...
void RotateEngine::run() {
	...
	*/
    ...
    rotate_threadarg_t* args = new rotate_threadarg_t[numthreads];
