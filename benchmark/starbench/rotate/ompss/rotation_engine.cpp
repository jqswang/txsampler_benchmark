...
*   Function: computeRow
*   --------------------
*   Computes the <row>th image scanline of the target image. Rewritten as a separate function for taskification.
*/
...
//==========================================================================================================================	
    ...
    for(int i = 0; i < target_h; i+=4) {
      #pragma omp parallel
      {
	#pragma omp single nowait
	{
	  //#pragma omp task
	  for(int j=i; j<i+4 && j<target_h ; j++) {
	    #pragma omp task
	    this->computeRow(j, target_w, x_offset_target, y_offset_target, x_offset_source, y_offset_source, rev_angle);
	  }
	}//end single
      }//end parallel
    }
