...
/* Structure used for handing arguments to processing threads */
struct threadarg {
    uint8_t* in;            // pointer to first pixel which this thread must process
    uint8_t* pY;   // Y
    uint8_t* pU;   // U
    uint8_t* pV;   // V
    int pixels_to_process;  // amount of pixels which this thread must process
    int tid;
};
