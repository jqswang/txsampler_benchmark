...
#define QUEUE_HEAD(node_t) \
	struct { \
		...
		node_t *tail; \
		pthread_mutex_t_wrapper mutex; \
		pthread_cond_t_wrapper not_empty; \
		pthread_cond_t_wrapper not_full; \
	}
#endif //RTM
...
#define QUEUE_INIT(queue)	\
	do {			\
		...
		pthread_mutex_init_wrapper(&(queue)->mutex, NULL); \
	} while (0)
...
#define QUEUE_INIT_DEPTH(queue,d)	\
	do {			\
		QUEUE_INIT(queue);	\
		(queue)->depth = d;	\
		pthread_cond_init_wrapper(&(queue)->not_empty, NULL); \
		pthread_cond_init_wrapper(&(queue)->not_full, NULL); \
	} while (0)
...
#define queue_destroy(queue)	\
	do {			\
		...
		pthread_mutex_destroy_wrapper(&(queue)->mutex); \
	} while (0)
...
#define queue_lock(queue)	pthread_mutex_lock_wrapper(&(queue)->mutex)

#define queue_unlock(queue)	pthread_mutex_unlock_wrapper(&(queue)->mutex)

#define queue_enqueue_sync(queue,node) \
	do {	\
		pthread_mutex_lock_wrapper(&(queue)->mutex); \
		queue_enqueue(queue,node);	\
		pthread_mutex_unlock_wrapper(&(queue)->mutex); \
	} while(0)
...
#define queue_enqueue_wait(queue, node) \
	do {\
		pthread_mutex_lock_wrapper(&(queue)->mutex); \
		while((queue)->count >= (queue)->depth) pthread_cond_wait_wrapper(&(queue)->not_full, &(queue)->mutex); \
		queue_enqueue(queue,node);	\
		if ((queue)->count > 0) pthread_cond_signal_wrapper(&(queue)->not_empty); \
		pthread_mutex_unlock_wrapper(&(queue)->mutex); \
	} while (0)
...
#define queue_dequeue_wait(queue, node) \
	do {\
		pthread_mutex_lock_wrapper(&(queue)->mutex); \
		while((queue)->count == 0) pthread_cond_wait_wrapper(&(queue)->not_empty, &(queue)->mutex); \
		queue_dequeue(queue,node);	\
		if ((queue)->count < (queue)->depth) pthread_cond_signal_wrapper(&(queue)->not_full); \
		pthread_mutex_unlock_wrapper(&(queue)->mutex); \
	} while (0)
