...
int dequeue(struct queue * que, int * fetch_count, void ** to_buf) {
      ...
      for ((*fetch_count) = 0; (*fetch_count) < ITEM_PER_FETCH; (*fetch_count) ++) {
        ...
        if (que->tail == que->head) {
          (*fetch_count) ++;
          //break;
          pthread_mutex_unlock(&que->mutex);//qwang
          return 0;
        }
      }
      pthread_mutex_unlock(&que->mutex);
      pthread_mutex_lock(&que->mutex);
      for (; (*fetch_count) < ITEM_PER_FETCH *2/3; (*fetch_count) ++) {
        to_buf[(*fetch_count)] = que->data[que->tail];
        que->tail ++;
        if (que->tail == que->size) que->tail = 0;
        if (que->tail == que->head) {
      ...
      pthread_mutex_unlock(&que->mutex);
      pthread_mutex_lock(&que->mutex);
      for (; (*fetch_count) < ITEM_PER_FETCH; (*fetch_count) ++) {
        to_buf[(*fetch_count)] = que->data[que->tail];
        que->tail ++;
        if (que->tail == que->size) que->tail = 0;
        if (que->tail == que->head) {
...
int enqueue(struct queue * que, int * fetch_count, void ** from_buf) {
  ...
  if ((*fetch_count) == -9999) {
   ...
   #define XX 7
    int tmp = (*fetch_count)>XX?(*fetch_count-XX):0;
    while ((*fetch_count) > tmp) {
      (*fetch_count) --;
     ...
     pthread_mutex_lock(&que->mutex);
    tmp = (*fetch_count)>XX?(*fetch_count-XX):0;
    while ((*fetch_count) > tmp) {
      (*fetch_count) --;
      que->data[que->head] = from_buf[(*fetch_count)];
      que->head ++;
