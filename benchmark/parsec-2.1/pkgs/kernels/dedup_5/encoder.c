...
__thread struct pContent * value_memory = NULL;
__thread send_buf_item * item_memory = NULL;
__thread send_body * body_memory = NULL;
__thread u_char * content_memory = NULL;
__thread u_char * sha1_memory = NULL;
...
#ifdef PARALLEL
      #ifdef RTM
      TM_LOCK_T *ht_lock = hashtable_getlock(cache, (void *)key);
      #else
...
send_buf_item * sub_ChunkProcess(data_chunk chunk) {
  ...
  u_char * key;
/*
  static __thread struct pContent * value_memory = NULL;
  static __thread send_buf_item * item_memory = NULL;
  static __thread send_body * body_memory = NULL;
  static __thread u_char * content_memory = NULL;
  static __thread u_char * sha1_memory = NULL;
*/
  content_memory = (u_char * )malloc(chunk.len + 1);
  ...
  pthread_mutex_unlock(ht_lock);
#endif
    value_memory = (struct pContent *)malloc(sizeof(struct pContent));
    if( value_memory == NULL) {
      EXIT_TRACE("Memory allocation failed.\n");
    }
    sha1_memory = (u_char *)malloc(SHA1_LEN);
    if(sha1_memory == NULL) {
      EXIT_TRACE("Memory allocation failed.\n");
    }
  ...
  pthread_mutex_unlock(ht_lock);
    ...
    item_memory = (send_buf_item *)malloc(sizeof(send_buf_item));
    if( item_memory  == NULL) {
      EXIT_TRACE("Memory allocation failed.\n");
    }
    body_memory = (send_body *)malloc(sizeof(send_body));
    if( body_memory  == NULL) {
      EXIT_TRACE("Memory allocation failed.\n");
    }
    ...
    case TYPE_COMPRESS:
      ...
      if (body->cid == reassemble_count && body->anchorid == anchor_count) {
...
#ifdef PARALLEL
        #ifdef RTM
        TM_LOCK_T *ht_lock = hashtable_getlock(cache, (void *)item->content);
        #else
        ...
        if ((entry = hashtable_search(cache, (void *)item->content)) != NULL) {
	      ...
	      pthread_mutex_unlock(ht_lock); //qwang

              write_file(fd, TYPE_COMPRESS, seq_count, value->len, value->content);
              pthread_mutex_lock(ht_lock); //qwang
              value->len = seq_count;
...
#ifdef PARALLEL
            #ifdef RTM
            TM_LOCK_T *ht_lock = hashtable_getlock(cache, (void *)hele->content);
            #else
...
#ifdef BALANCED
  struct thread_args chunk_thread_args[1];
  for (i=0; i < 1; i++)
#else
...
#ifdef BALANCED
  struct thread_args anchor_thread_args[1];
  for (i = 0; i < 1; i ++)
#else
...
#ifdef BALANCED
  struct thread_args compress_thread_args[10];
  for (i = 0; i < 10; i ++)
#else
